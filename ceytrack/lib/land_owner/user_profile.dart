// user_details.dart — MODERN BLUE THEME + COMPACT UI (LAND OWNER)
...
class _UserDetailsState extends State<UserDetails> {
  ...
  void _fetchHeaderData() async {
    ...
    try {
      ...
      if (landDoc.exists) {
        ...
        setState(() {
          _landName = user.email?.split('@')[0] ?? 'Land';
        });
      }
    } catch (e) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Manage User Details',
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser == null) {
      return Scaffold(body: Center(child: Text("User not logged in.", style: TextStyle(fontSize: screenWidth * 0.04))));
    }
...
// ===================== USER PROFILE CONTENT (BLUE THEME) =====================
...
class _UserProfileContentOnlyState extends State<UserProfileContentOnly> {
          ...
          children: [
            ...
            if (_profileImageUrl != null || _pickedImageFile != null)
              ListTile(
                ...
                title: Text('Remove Current Photo', style: TextStyle(color: AppColors.errorRed, fontSize: isSmall ? 14 : 16)),
                subtitle: Text('Remove profile picture', style: TextStyle(color: AppColors.errorRed, fontSize: isSmall ? 12 : 14)),
                onTap: () { Navigator.pop(ctx); _removeProfileImage(); },
              ),
            ListTile(
              leading: Icon(Icons.close, color: AppColors.primaryBlue),
  ...
  // ===================== COMPACT UI BUILDERS (BLUE THEME) =====================
  @override
  Widget build(BuildContext context) {
    final isSmall = widget.screenWidth < 360;
          ...
          children: [
            if (_statusMessage != null)
              Container(
                ...
                decoration: BoxDecoration(
                  color: _statusMessage!.toLowerCase().contains('success') ? AppColors.lightBlue.withOpacity(0.1) : AppColors.accentRed.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: _statusMessage!.toLowerCase().contains('success') ? AppColors.lightBlue : AppColors.accentRed),
                ),
                child: Text(_statusMessage!, style: TextStyle(fontSize: isSmall ? 12 : 13)),
              ),
            
            // Profile Picture (compact, blue theme)
                ...
                child: _uploadingImage
                    ...
                    : CircleAvatar(
                        ...
                        child: _getProfileImage() == null
                            ? Icon(Icons.person, size: size * 0.4, color: AppColors.primaryBlue)
                            : null,
                      ),
              ),
              Positioned(
        ...
        if (_pickedImageFile != null)
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Icon(Icons.image, size: 14, color: AppColors.primaryBlue),
  ...
  Widget _buildTextField(TextEditingController ctrl, String hint, String? Function(String?)? validator, bool isSmall, [TextInputType? type]) => Container(
    ...
    child: TextFormField(
      ...
      decoration: InputDecoration(
        hintText: hint,
        hintStyle: TextStyle(fontSize: isSmall ? 13 : 14, color: AppColors.textTertiary),
...
class BlueGradientButton extends StatelessWidget {
  ...
  const BlueGradientButton({required this.text, this.onPressed, this.isEnabled = true, super.key});
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          gradient: isEnabled
              ? const LinearGradient(colors: [AppColors.primaryBlue, AppColors.mediumBlue])
              : LinearGradient(colors: [Colors.grey.shade400, Colors.grey.shade300]),
          boxShadow: isEnabled ? [BoxShadow(color: AppColors.primaryBlue.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))] : null,
        ),
